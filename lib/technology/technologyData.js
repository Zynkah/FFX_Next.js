...
export const get_technology_by_name = (role) => {
  return data.technology
    .filter((name) => name.role == role)
    .sort((a, b) => a.key - b.key);
};

export const get_technology_by_role = (role) => {
  return data.technology.filter((name) => name.role == role).at(0);
};
